    ...
    <footer className="bg-charcoal text-white font-sans">
      ...
      <div className="border-b border-white/10 py-16 px-5 md:px-8">
        <div className="max-w-xl mx-auto text-center">
          <h3 className="font-serif text-2xl md:text-3xl mb-3">Healthy Skin Starts With the Right Protocol</h3>
          <p className="text-white/60 text-sm leading-relaxed mb-8">
            Join the Skin Protocol RX community for skincare education, product guidance, new releases, and exclusive offers.
          </p>
          {submitted ? (
            <p className="text-green font-medium">You&apos;re on the list. Welcome to the protocol.</p>
          ) : (
            <form onSubmit={handleSubmit} className="flex gap-3 max-w-sm mx-auto">
              <input
                ...
                required
                className="flex-1 bg-white/10 border border-white/20 rounded px-4 py-2.5 text-sm text-white placeholder:text-white/40 focus:outline-none focus:border-green transition-colors"
              />
              <button
                type="submit"
                className="bg-green hover:bg-green-dark text-white px-5 py-2.5 text-sm font-medium rounded transition-colors whitespace-nowrap"
              >
                Join the Protocol
              </button>
            </form>
      ...
      <div className="max-w-7xl mx-auto px-5 md:px-8 py-14 grid grid-cols-2 md:grid-cols-4 gap-10">
        ...
        <div className="col-span-2 md:col-span-1">
          <div className="mb-5">
            <div className="font-serif text-lg font-semibold tracking-tight">Skin Protocol</div>
            <div className="text-[10px] tracking-[0.25em] uppercase text-green font-medium">RX</div>
          </div>
          ...
          <div className="flex gap-4">
            {['instagram', 'tiktok', 'facebook'].map(s => (
              <button key={s} className="text-white/40 hover:text-white transition-colors text-xs uppercase tracking-widest">{s.slice(0,2).toUpperCase()}</button>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <ul className="space-y-3">
            {[
              ...
              <li key={l.label}>
                <button onClick={() => onNavigate(l.page)} className="text-sm text-white/60 hover:text-white transition-colors">{l.label}</button>
              </li>
            ))}
        ...
        {/* Support */}
        <div>
          <h4 className="text-xs tracking-widest uppercase text-white/40 mb-5">Support</h4>
          <ul className="space-y-3">
