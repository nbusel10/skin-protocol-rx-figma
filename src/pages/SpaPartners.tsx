...
export default function SpaPartners({ onNavigate }: SpaPartnersProps) {
          ...
          {[
            ...
            <div key={f.key}>
              ...
              <input
                ...
                className="w-full border border-gray-soft bg-white px-4 py-3 text-sm focus:outline-none focus:border-green transition-colors"
              />
            </div>
          ))}
          <div>
        ...
        <div className="space-y-4">
          ...
          <label className="flex items-start gap-3 cursor-pointer mt-4">
            <div
              ...
              style={{ borderColor: form.agreed ? '#4D954C' : '#E8E8E8', backgroundColor: form.agreed ? '#4D954C' : 'transparent' }}
  ...
  if (submitted) {
    return (
      <div className="bg-ivory min-h-screen flex items-center justify-center font-sans px-5">
        <div className="text-center max-w-md">
          <div className="w-12 h-12 bg-green flex items-center justify-center mx-auto mb-6">
          ...
          <button onClick={() => onNavigate('home')} className="text-sm text-green underline underline-offset-2">Return to Home</button>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="bg-ivory min-h-screen font-sans">
        <div className="max-w-2xl mx-auto px-5 md:px-8 py-16">
          <button onClick={() => setShowForm(false)} className="flex items-center gap-2 text-sm text-charcoal/40 hover:text-charcoal mb-10 transition-colors">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
              <path d="M19 12H5M12 19l-7-7 7-7" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
            Back
          </button>
          <p className="text-[11px] tracking-[0.3em] uppercase text-green mb-2">Partnership Application</p>
          <h1 className="font-serif text-3xl md:text-4xl text-charcoal mb-10">Spa Partner Application</h1>

          {/* Progress */}
          <div className="flex gap-1.5 mb-10">
            {FORM_STEPS.map((_, i) => (
              <div key={i} className="h-1 flex-1 transition-all duration-300"
                style={{ backgroundColor: i <= formStep ? '#4D954C' : '#E8E8E8' }} />
            ))}
          </div>

          <div className="bg-white p-8 border border-gray-soft">
            <h2 className="font-serif text-xl text-charcoal mb-6">{FORM_STEPS[formStep].title}</h2>
          ...
          <div className="flex items-center justify-between mt-6">
            ...
            {formStep < FORM_STEPS.length - 1 ? (
              <button
                ...
                className="bg-charcoal text-white px-8 py-3 text-sm font-medium hover:bg-charcoal/80 transition-colors"
              >Continue →</button>
            ) : (
              <button
                onClick={() => setSubmitted(true)}
                disabled={!form.agreed}
                className="bg-green text-white px-8 py-3 text-sm font-medium hover:bg-green-dark disabled:opacity-40 transition-colors"
    ...
    <div className="bg-ivory font-sans">
      ...
      <section className="relative min-h-[60vh] flex items-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-5 md:px-8 py-24">
          <p className="text-[11px] tracking-[0.3em] uppercase text-green mb-5">For Spas & Estheticians</p>
          <h1 className="font-serif text-4xl md:text-6xl text-white max-w-2xl mb-6">
            Bring a Better Protocol to Your Treatment Room
          </h1>
          <p className="text-white/70 max-w-xl mb-10 leading-relaxed">
          ...
          <div className="flex flex-wrap gap-4">
            <button
              ...
              className="bg-green text-white px-8 py-4 text-sm font-medium hover:bg-green-dark transition-colors"
            >
              Apply to Become a Spa Partner
            </button>
            <button className="border border-white/40 text-white px-8 py-4 text-sm font-medium hover:border-white transition-colors">
              Spa Partner Login
            </button>
          </div>
        </div>
      ...
      {/* Benefits */}
      <section className="bg-white py-20 px-5 md:px-8 border-b border-gray-soft">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-14">
            <p className="text-[11px] tracking-[0.3em] uppercase text-green mb-3">Why Partner With Us</p>
            <h2 className="font-serif text-3xl md:text-4xl text-charcoal">Spa Partnership Benefits</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {BENEFITS.map(b => (
              <div key={b.title} className="border border-gray-soft p-6 hover:border-green transition-colors group">
                <div className="text-2xl text-green mb-4 font-serif">{b.icon}</div>
                <h3 className="font-medium text-charcoal mb-2 group-hover:text-green transition-colors text-sm">{b.title}</h3>
                <p className="text-xs text-charcoal/50 leading-relaxed">{b.body}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 px-5 md:px-8">
        <div className="max-w-3xl mx-auto text-center">
          <p className="text-[11px] tracking-[0.3em] uppercase text-green mb-5">The Bigger Picture</p>
          <h2 className="font-serif text-3xl md:text-4xl text-charcoal mb-6">Why It Matters to Your Practice</h2>
      ...
      {/* How it works */}
      <section className="bg-charcoal py-20 px-5 md:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-14">
            <p className="text-[11px] tracking-[0.3em] uppercase text-green mb-3">The Process</p>
            <h2 className="font-serif text-3xl md:text-4xl text-white">How the Partnership Works</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {PROCESS.map(p => (
              <div key={p.step} className="flex flex-col gap-4">
                <div className="font-serif text-4xl text-green/40">{p.step}</div>
      ...
      {/* Testimonials */}
      <section className="py-20 px-5 md:px-8 bg-white border-b border-gray-soft">
        <div className="max-w-7xl mx-auto">
          <p className="text-[11px] tracking-[0.3em] uppercase text-green mb-12 text-center">Professional Reviews</p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {SPA_TESTIMONIALS.map((t, i) => (
              <div key={i} className="bg-ivory p-8 flex flex-col gap-4">
                <p className="font-serif text-base italic text-charcoal leading-relaxed">&ldquo;{t.text}&rdquo;</p>
                <div className="mt-auto pt-4 border-t border-gray-soft">
                  <p className="text-sm font-medium text-charcoal">{t.name}</p>
                  <p className="text-xs text-charcoal/40">{t.business}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-20 px-5 md:px-8 text-center bg-green-light">
        <p className="text-[11px] tracking-[0.3em] uppercase text-green mb-4">Ready to Partner?</p>
        <h2 className="font-serif text-3xl md:text-4xl text-charcoal mb-4">Give Your Clients a Protocol They Can Continue at Home</h2>
        <p className="text-charcoal/60 mb-10 max-w-xl mx-auto">Join Skin Protocol RX&apos;s professional partner network and add a coordinated home-care line to your practice.</p>
        <button
          onClick={() => setShowForm(true)}
          className="bg-charcoal text-white px-10 py-4 text-sm font-medium hover:bg-charcoal/80 transition-colors"
        >
