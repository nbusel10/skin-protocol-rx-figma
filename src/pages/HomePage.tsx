...
function ProductCard({ product, onNavigate }: { product: typeof PRODUCTS[0]; onNavigate: (page: Page, id?: string) => void }) {
  ...
  return (
    ...
    >
      {/* Image */}
      <div className="relative aspect-square bg-ivory-dark overflow-hidden mb-4">
        <img
          src={product.image}
          alt={product.name}
        ...
        >
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#252525" strokeWidth="1.8">
            <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" />
          </svg>
        </button>
        {/* Quick add */}
        <div className={`absolute bottom-0 left-0 right-0 transition-all duration-300 ${hovered ? 'translate-y-0 opacity-100' : 'translate-y-2 opacity-0'}`}>
          <button
            className="w-full bg-charcoal text-white text-xs tracking-widest uppercase py-3 hover:bg-green transition-colors duration-200 font-medium"
            onClick={e => { e.stopPropagation() }}
          >
            Quick Add
          </button>
      ...
      <div className="flex flex-col gap-1.5 flex-1">
        <div className="flex items-start justify-between gap-2">
          <div>
            <p className="text-[11px] tracking-widest uppercase text-charcoal/40 mb-0.5 font-sans">
              {product.categories[0]}
            </p>
            <h3 className="font-sans text-sm font-medium text-charcoal leading-snug">{product.name}</h3>
          </div>
          <p className="font-sans text-sm font-semibold text-charcoal whitespace-nowrap">${product.price}</p>
        </div>
        <p className="text-xs text-charcoal/50 leading-relaxed">{product.tagline}</p>
  ...
  return (
    <div className="bg-ivory font-sans">
      ...
      <section className="relative min-h-[90vh] grid grid-cols-1 lg:grid-cols-2">
        ...
        {/* Right: image */}
        <div className="relative order-1 lg:order-2 h-72 lg:h-auto bg-green-light overflow-hidden">
          <img
            src="https://images.unsplash.com/photo-1674932668403-33398b81c92f?w=1200&h=1400&fit=crop&auto=format"
      ...
      {/* ── BRAND PROMISE ────────────────────────────────────────────── */}
      <section className="border-t border-gray-soft bg-white py-16 px-5 md:px-8">
        <div className="max-w-7xl mx-auto grid grid-cols-2 lg:grid-cols-4 gap-10">
          {BRAND_PROMISES.map(p => (
            <div key={p.title} className="flex flex-col items-center text-center gap-4">
              ...
              <h3 className="font-sans text-sm font-semibold text-charcoal tracking-wide">{p.title}</h3>
              <p className="text-xs text-charcoal/50 leading-relaxed">{p.body}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-20 px-5 md:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
            <div>
              <p className="text-[11px] tracking-[0.3em] uppercase text-green mb-3">Find Your Focus</p>
              <h2 className="font-serif text-3xl md:text-4xl text-charcoal">Shop by Skin Concern</h2>
            </div>
            <button
              onClick={() => onNavigate('shop')}
              className="text-sm text-charcoal underline underline-offset-4 hover:text-green transition-colors"
            >
              View All Products →
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
            {SKIN_CONCERNS.map(c => (
              <button
                key={c.id}
                onClick={() => onNavigate('shop')}
                className="group relative bg-white border border-gray-soft hover:border-green p-6 text-left transition-all duration-200 hover:shadow-md"
              >
                <div className="text-2xl text-charcoal/20 group-hover:text-green transition-colors mb-3 font-serif">{c.icon}</div>
                <h3 className="text-sm font-medium text-charcoal mb-1 group-hover:text-green transition-colors">{c.label}</h3>
                <p className="text-xs text-charcoal/40 leading-relaxed hidden lg:block">{c.description}</p>
              </button>
            ))}
          </div>
        </div>
      ...
      {/* ── FEATURED PRODUCTS ────────────────────────────────────────── */}
      <section className="bg-white py-20 px-5 md:px-8 border-t border-gray-soft">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
            <div>
              <p className="text-[11px] tracking-[0.3em] uppercase text-green mb-3">Professional Favorites</p>
              <h2 className="font-serif text-3xl md:text-4xl text-charcoal">Featured Products</h2>
            </div>
            <button onClick={() => onNavigate('shop')} className="text-sm text-charcoal underline underline-offset-4 hover:text-green transition-colors">
              Shop All →
            </button>
          </div>
      ...
      {/* ── BUILD YOUR PROTOCOL ──────────────────────────────────────── */}
      <section className="py-24 px-5 md:px-8 bg-green-light">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div>
            <p className="text-[11px] tracking-[0.3em] uppercase text-green mb-4">Personalized Routine</p>
            <h2 className="font-serif text-3xl md:text-4xl text-charcoal mb-6">
              A Better Routine Starts With a Better Protocol
            </h2>
            <p className="text-base text-charcoal/60 leading-relaxed mb-10 max-w-lg">
            ...
            <button
              onClick={() => onNavigate('protocol')}
              className="bg-green text-white px-8 py-4 text-sm tracking-wide font-medium hover:bg-green-dark transition-colors"
            >
              Build My Protocol
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {PROTOCOL_STEPS.map(s => (
              <div key={s.step} className="bg-white p-5 flex flex-col gap-2">
                <span className="text-[10px] tracking-[0.25em] uppercase text-green font-medium">Step {s.step}</span>
                <h3 className="font-serif text-xl text-charcoal">{s.name}</h3>
                <p className="text-xs text-charcoal/50 leading-relaxed">{s.description}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="relative overflow-hidden min-h-[500px] flex items-center">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-5 md:px-8 py-24 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div>
            <p className="text-[11px] tracking-[0.3em] uppercase text-green mb-5">Our Philosophy</p>
          ...
          <div className="flex flex-col gap-4">
            {['Our Standards', 'Ingredient Glossary', 'Product Education'].map(l => (
              <button
                key={l}
                className="flex items-center justify-between text-white/80 hover:text-white border-b border-white/20 pb-4 text-sm font-medium group transition-colors"
              >
                {l}
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="relative">
            <div className="aspect-[4/5] bg-ivory overflow-hidden">
              <img
                src="https://images.unsplash.com/photo-1581182815808-b6eb627a8798?w=900&h=1100&fit=crop&auto=format"
                alt="Natural skin portrait"
                className="w-full h-full object-cover"
              />
            </div>
            {/* Decorative inset card */}
            <div className="absolute -bottom-6 -right-6 bg-ivory border border-gray-soft p-6 max-w-xs hidden md:block">
              <p className="text-[10px] tracking-[0.3em] uppercase text-green mb-2">Est. 2019</p>
              <p className="font-serif text-lg text-charcoal leading-snug">Created in New York by a family who believes healthy skin doesn&apos;t need more than what works.</p>
            </div>
          </div>
          <div className="lg:pl-8">
            <p className="text-[11px] tracking-[0.3em] uppercase text-green mb-5">Our Story</p>
            <h2 className="font-serif text-3xl md:text-4xl text-charcoal mb-6">
            ...
            >
              <span className="border-b border-charcoal pb-0.5 group-hover:border-green group-hover:text-green transition-colors">Our Story</span>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="group-hover:translate-x-1 transition-transform text-charcoal group-hover:text-green">
                <path d="M5 12h14M12 5l7 7-7 7" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </button>
          </div>
      ...
      {/* ── SPA PARTNERSHIP ──────────────────────────────────────────── */}
      <section className="bg-charcoal py-24 px-5 md:px-8">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div>
            <p className="text-[11px] tracking-[0.3em] uppercase text-green mb-5">For Professionals</p>
            <h2 className="font-serif text-3xl md:text-4xl lg:text-5xl text-white mb-6">
            ...
            <div className="flex flex-wrap gap-4">
              <button
                ...
                className="bg-green text-white px-7 py-3.5 text-sm font-medium hover:bg-green-dark transition-colors"
              >
                Explore Spa Partnerships
              </button>
              <button className="border border-white/30 text-white px-7 py-3.5 text-sm font-medium hover:border-white transition-colors">
                Spa Partner Login
              </button>
            </div>
          </div>
          <div className="relative aspect-video lg:aspect-[4/5] bg-white/5 overflow-hidden">
            <img
      ...
      {/* ── TESTIMONIALS ─────────────────────────────────────────────── */}
      <section className="py-24 px-5 md:px-8 bg-ivory">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            <p className="text-[11px] tracking-[0.3em] uppercase text-green mb-4">Real Results</p>
            <h2 className="font-serif text-3xl md:text-4xl text-charcoal mb-8">What People Are Saying</h2>
            {/* Tabs */}
            <div className="inline-flex border border-gray-soft bg-white">
              <button
                onClick={() => setTestimonialTab('customer')}
                className={`px-6 py-2.5 text-sm font-medium transition-colors ${testimonialTab === 'customer' ? 'bg-charcoal text-white' : 'text-charcoal/50 hover:text-charcoal'}`}
              >
                Customer Results
              </button>
              <button
                onClick={() => setTestimonialTab('pro')}
                className={`px-6 py-2.5 text-sm font-medium transition-colors ${testimonialTab === 'pro' ? 'bg-charcoal text-white' : 'text-charcoal/50 hover:text-charcoal'}`}
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {testimonials.map((t, i) => (
              <div key={i} className="bg-white p-8 flex flex-col gap-5">
                ...
                <p className="font-serif text-base leading-relaxed text-charcoal italic">&ldquo;{t.text}&rdquo;</p>
      ...
      <section className="py-20 px-5 md:px-8 bg-white border-t border-gray-soft">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
            <h2 className="font-serif text-3xl md:text-4xl text-charcoal">The Complete Collection</h2>
            <button onClick={() => onNavigate('shop')} className="text-sm text-charcoal underline underline-offset-4 hover:text-green transition-colors">
              Shop All Products →
            </button>
          </div>
